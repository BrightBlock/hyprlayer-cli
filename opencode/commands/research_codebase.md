...
```bash
hyprlayer telemetry skill-start --skill research_codebase
```

(End of skill — substitute the token printed above:)

```bash
hyprlayer telemetry skill-end --skill research_codebase --session <token>
```
<!-- /hyprlayer:telemetry-beacon -->
...
3. **Delegate to sub-agents for comprehensive research:**
   ...
   **For JIRA tickets (if relevant):**
   ...
   - Then use `@codebase-analyzer` and `@thoughts-analyzer` on the most promising findings to document how they work
