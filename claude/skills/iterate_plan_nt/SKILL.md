...
The no-thoughts variant — no thoughts-directory agents, no `archivist`.
...
orchestration:
  owns: [input-parsing, scope, surgical-edits, user-confirmation, persistence]
  ...
  steps:
    ...
    - id: map-areas
      ...
      because: >
        ...
        message. The reject rule is the mechanical half of "which directories
        to focus on": it catches "the backend", not a real path aimed at the
        wrong area, which stays your judgment.
...
conventions:
  ...
  success-criteria:
    ...
    automated:
      ...
      examples: [make test, npm run lint, files-that-should-exist, compilation, type-checking]
    manual:
      needs:  human-testing
