...
Hyprlayer's Notion backend relies on the **agent tool's connector** (Claude.ai's Notion integration, surfaced as `mcp__claude_ai_Notion__notion-*`). Hyprlayer does *not* register its own Notion MCP server — `src/backends/notion.rs` is explicit about this, because a hyprlayer-registered server would shadow the working connector. Total budget: ~5–15s.
...
## 2. Duplicate MCP registration (shadow risk)

- Inspect `~/.claude.json` `mcpServers` (and any plugin MCP configs). ❌ if any entry would route `notion-*` calls somewhere other than the Claude.ai connector — a hyprlayer- or third-party-registered Notion server will shadow the connector and silently produce auth failures.
- Older sessions in this user's transcripts have surfaced both `mcp__claude_ai_Notion__notion-*` and `mcp__<uuid>__notion-*` simultaneously; the UUID-prefixed one is the failure mode here.
- Remediation: remove the stale registration from `~/.claude.json` `mcpServers` and restart the agent.
...
- From the database response, capture the **data source ID** (the `data_source_url`'s ID, looks like `53b5216f-…`). Step 6 needs it as the `data_source_id` parent.
...
1. Single call: `mcp__claude_ai_Notion__notion-create-pages` with:
   - `parent`: when `databaseId` is set, `{type: "data_source_id", data_source_id: <data_source_id from step 5>}`; otherwise `{type: "page_id", page_id: <parentPageId>}` (and skip the per-row properties block — non-database pages only accept `title`).
   - `pages`: a single-element array. For database parents:
     ...
     ```
   - For non-database parents: only `{title: "__hyprlayer_doctor_<unix_ts>", content: "hyprlayer_doctor verify"}`.
...
## 7. Latency baseline (optional)

Only when the dispatcher requested a full report. Sum wall-clock of steps 1, 3, 4, 6. If > 20s, note that Notion's API is degraded right now — operations will feel slow, but it's not a setup problem.
