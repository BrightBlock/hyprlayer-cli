...
Pick the invocation for your platform (Claude's system context tells you which one):

**macOS / Linux** (run via the Bash tool):

```bash
bash "$HOME/.claude/skills/code_review/scripts/check-hyprlayer-version.sh" 1.5.2 || exit 1
```

**Windows** (run via the Bash tool with PowerShell as the executor — Windows users typically run Claude Code in PowerShell, not git-bash):

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "$HOME\.claude\skills\code_review\scripts\check-hyprlayer-version.ps1" 1.5.2
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
```

The script (in either form) prints an install/upgrade hint with the correct package manager and exits non-zero if `hyprlayer` is missing or older than the required version. Both variants are cwd-independent.
...
Pick the invocation for your platform:

**macOS / Linux**:

```bash
EFFORT=high  # or xhigh
_REPO_ROOT="$_REPO_ROOT" bash "$HOME/.claude/skills/code_review/scripts/run-codex.sh" "$PROMPT" "$EFFORT" ${MODEL:-}
EXIT=$?
```

Use a Bash `timeout` of 660000ms on the outer call so the script's inner `timeout 600` fires first when codex hangs.

**Windows**:

```powershell
$env:_REPO_ROOT = $_REPO_ROOT
$args = @('-Prompt', $PROMPT, '-Effort', 'high')  # or 'xhigh'
if ($MODEL) { $args += @('-Model', $MODEL) }
powershell -NoProfile -ExecutionPolicy Bypass -File "$HOME\.claude\skills\code_review\scripts\run-codex.ps1" @args
$EXIT = $LASTEXITCODE
```

Windows PowerShell does not ship a native `timeout` — the script lets codex itself enforce request timeouts and surfaces a stall through codex's own non-zero exit, instead of wrapping with `timeout 600`. For bounded execution, set the Bash tool's outer timeout to ~600000ms.

> **Note:** both variants pipe codex's `--json` output through `hyprlayer codex stream`, which prints `[codex thinking]`, `[codex ran]`, agent messages, and token totals. They capture stderr and print a one-line auth hint when it looks like an auth/login error.
